                ...
                <GlassCard>
                    {dashboardMode === 'grid' ? (
                        ...
                        <div className="space-y-3">
                            {filteredNotes.map(note => (
                                <div key={note.id} onClick={() => { setCurrentNote(note); setView('editor'); }}
                                    className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/50 border border-transparent hover:border-primary-200 transition-all cursor-pointer group">
                                    <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center text-xl shadow-sm">{note.icon || '📝'}</div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-bold text-slate-800 truncate">{note.title || 'Untitled'}</h3>
                                        <p className="text-xs text-slate-500 truncate">{note.content.replace(/<[^>]*>/g, '') || 'No additional text'}</p>
                                    </div>
                                    <div className="text-xs text-slate-400 font-medium whitespace-nowrap">{new Date(note.updatedAt).toLocaleDateString()}</div>
                                    <button onClick={(e) => handleDeleteNote(e, note.id)} className="p-2 text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></button>
                                </div>
                            ))}
                        </div>
                    )}
