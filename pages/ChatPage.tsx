...
export const ChatPage: React.FC<ChatViewProps> = ({
  ...
  return (
    <PageContainer scrollable={false}>
      <GlassCard className="flex flex-col h-full p-0 overflow-hidden" noPadding>

        {/* Top Bar */}
        <div className="h-20 flex items-center justify-between px-6 md:px-8 border-b border-white/50 bg-white/40 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-3 bg-white/50 hover:bg-white transition-all px-4 py-2 rounded-2xl cursor-pointer border border-white/60 shadow-sm hover:shadow-md"
              >
        ...
        {/* Chat History Area */}
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6 custom-scrollbar scroll-smooth bg-white/30">
          <div className="max-w-3xl mx-auto space-y-8 pb-4">
                ...
                {activeAgent.tools && activeAgent.tools.length > 0 && (
                  <div className="flex flex-wrap justify-center gap-2">
                    {activeAgent.tools.map(tool => (
                      <Badge key={tool} variant="outline" className="bg-white/60">
                        {tool}
                      </Badge>
                    ))}
                  </div>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex max-w-[85%] md:max-w-[75%] gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row group'}`}>
                  ...
                  <div className={`flex flex-col gap-1 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div
                      className={`px-6 py-4 text-base leading-7 whitespace-pre-wrap shadow-sm backdrop-blur-sm
                        ${msg.role === 'user'
                          ? 'bg-slate-900 text-white rounded-[24px] rounded-tr-sm shadow-xl shadow-slate-900/10'
                          : 'bg-white/80 border border-white/60 text-slate-800 rounded-[24px] rounded-tl-sm shadow-[0_4px_20px_rgb(0,0,0,0.03)]'}`}
                    >
                      {msg.content}
                    </div>
            ...
            {isThinking && (
              <div className="flex w-full justify-start items-center gap-4 pl-14">
                <div className="flex gap-4 flex-row">
                  <div className="bg-white/60 backdrop-blur-md px-6 py-4 rounded-[24px] rounded-tl-sm flex items-center gap-2 border border-white/50 shadow-sm">
        ...
        {/* Input Area */}
        <div className="p-6 md:p-8 bg-white/40 border-t border-white/50 shrink-0">
